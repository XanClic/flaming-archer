...
dake::mat4 dake::mat4::operator*(const dake::mat4 &om) const
{
    mat4 m(*this);
    m *= om;
    return m;
...
dake::mat4 &dake::mat4::translate(const dake::vec3 &vec)
{
#ifdef X64_ASSEMBLY
    __asm__ __volatile__ (
            "pshufd $0x00,%0,%0;"
...
dake::mat4 dake::mat4::translated(const dake::vec3 &vec) const
{
    return mat4(*reinterpret_cast<const dake::vec4 *>(&d[ 0]),
                *reinterpret_cast<const dake::vec4 *>(&d[ 4]),
                *reinterpret_cast<const dake::vec4 *>(&d[ 8]),
                *reinterpret_cast<const dake::vec4 *>(&d[12]) +
                *reinterpret_cast<const dake::vec4 *>(&d[ 0]) * vec.x() +
                *reinterpret_cast<const dake::vec4 *>(&d[ 4]) * vec.y() +
                *reinterpret_cast<const dake::vec4 *>(&d[ 8]) * vec.z());
}

dake::mat4 &dake::mat4::rotate(float angle, const dake::vec3 &axis)
{
    if (!angle)
        return *this;
    ...
    *this *= *reinterpret_cast<dake::mat4 *>(rm);
#else
    float _00 = x * x * omc +     c;
...
dake::mat4 dake::mat4::rotated(float angle, const dake::vec3 &axis) const
{
    if (!angle)
        return dake::mat4(*this);
    ...
    return *this * *reinterpret_cast<dake::mat4 *>(rm);
#else
    float _00 = x * x * omc +     c;
    float _01 = x * y * omc - z * s;
    float _02 = x * z * omc + y * s;
...
dake::mat4 &dake::mat4::scale(const dake::vec3 &fac)
{
#ifdef X64_ASSEMBLY
    __asm__ __volatile__ (
            "pshufd $0x00,%0,%0;"
...
dake::mat4 dake::mat4::scaled(const dake::vec3 &fac) const
{
    return mat4(*reinterpret_cast<const dake::vec4 *>(&d[ 0]) * fac.x(),
                *reinterpret_cast<const dake::vec4 *>(&d[ 4]) * fac.y(),
                *reinterpret_cast<const dake::vec4 *>(&d[ 8]) * fac.z(),
                *reinterpret_cast<const dake::vec4 *>(&d[12]));
}
...
void dake::mat4::transposed_invert(void)
{
#ifdef X64_ASSEMBLY
    __asm__ __volatile__ (
            "rcpss  %1,%1;"
...
dake::vec4 dake::mat4::operator*(const dake::vec4 &v) const
{
    return dake::vec4(
...
/*
dake::vec3 dake::mat3::operator*(const dake::vec3 &v) const
{
    return vec3(
